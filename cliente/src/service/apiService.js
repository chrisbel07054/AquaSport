...
export const api = {
    ...
    torneo: {
        ...
        inscribirse: (torneoId, usuarioId, token) => request(`/torneo/inscripcion/${torneoId}`, 'POST', {usuarioId}, token),
        changeStatus: (torneoId, data, token) => request(`/torneo/cambiar-estado/${torneoId}`, 'PUT', data, token),
        updateTorneo: (torneoId, data, token) => request(`/torneo/${torneoId}`, 'PUT', data, token),
    },
    testimonio: {
    ...
    usuario: {
        ...
        getTorneos: (userId, token) => request(`/usuario/torneos/${userId}`, 'GET', null, token),
        getTestimonios: (userId, token) => request(`/usuario/testimonios/${userId}`, 'GET', null, token),
        updateProfile: (userId, data, token) => request(`/usuario/${userId}`, 'PUT', data, token),
    },
    contacts: {
