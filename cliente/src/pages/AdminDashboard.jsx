...
import { User, LogOut, Users, Trophy, MessageSquare, Plus, Edit, X, Check, Star } from "lucide-react"
...
const AdminDashboard = () => {
  ...
  const handleSaveTorneo = async (updatedTorneo) => {
    try {
      ...
      await api.torneo.updateTorneo(updatedTorneo.id, updatedTorneo, user.token);
  ...
  const torneosCancelados = torneos.filter((t) => t.estado === "cancelado").length
  const totalUsuarios = usuarios.length
  const totalTestimonios = testimonios.length
  const totalInscritos = torneos.reduce((sum, torneo) => sum + torneo.inscritos, 0)
          ...
          <div className="bg-slate-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-white">{totalTorneos}</h3>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-xs text-green-400">{torneosActivos} activos</span>
                  <span className="text-xs text-slate-400">•</span>
                  <span className="text-xs text-red-400">{torneosCancelados} cancelados</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-white">{totalUsuarios}</h3>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-white">{totalInscritos}</h3>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-white">{totalTestimonios}</h3>
              </div>
            </div>
          </div>
        </motion.div>
                ...
                <tbody className="divide-y divide-slate-700">
                  {torneos.map((torneo) => (
                    <tr key={torneo.id} className="hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-slate-300">{new Date(torneo.fecha).toISOString().split('T')[0]}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                ...
                <tbody className="divide-y divide-slate-700">
                  {usuarios.map((usuario) => (
                    <tr key={usuario.id} className="hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-slate-300 capitalize">{usuario.genero}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-slate-300">{usuario.edad} años</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-center">
                        <div className="text-sm text-slate-300">
                          {new Date(usuario.createdAt).toISOString().split('T')[0]}
                        </div>
                      </td>
                    </tr>
                  ))}
        ...
        >
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            {testimonios.slice(0, 6).map((testimonio) => (
              <div key={testimonio.id} className="bg-slate-700/30 rounded-lg p-4">
                <div className="flex items-center mb-2">
                  <div className="h-8 w-8 rounded-full bg-blue-600 flex items-center justify-center text-white mr-3">
                    <User className="h-4 w-4" />
                  </div>
                  <div>
                    <div className="text-sm font-medium text-white">{testimonio.Usuario.nombre}</div>
                    <div className="text-xs text-slate-400">{testimonio.createdAt ? new Date(testimonio.createdAt).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]}</div>
                  </div>
                </div>
                <p className="text-sm text-slate-300 line-clamp-3">{testimonio.comentario}</p>
                <div className="mt-2 flex items-center">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`h-4 w-4 ${
                        i < testimonio.calificacion ? "text-yellow-400 fill-current" : "text-slate-600"
                      }`}
                    />
                  ))}
                </div>
          ...
          <div className="p-4 border-t border-slate-700 text-center">
            <Link to="/testimonials" className="text-blue-400 hover:text-blue-400/80 text-sm">
